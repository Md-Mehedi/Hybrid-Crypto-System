...
class AES:
  ...
  state_matrix:List[List[BitVector]] = []
  ...
  def key_expansion(self) -> None:
    ...
    hex_key = self.text_to_hex(self.KEY)              # Convert the ASCII text string to Hexadecimal string
    self.w = self.hex_to_matrix(hex_key);             # Split by 2 character and make a 4x4 print_bv_matrix
    
    round_count = 0
    for i in range(4, 4*TOTAL_ROUND+4):               # By looping generate keys for different round
      temp = []
      if(i%4==0):
        temp = array_xor(self.w[i-4], self.g(self.w[i-1], round_count=round_count))   # Uses g() after 4 iteration
  ...
  def encrypt_segment(self, plain_text:str) -> str:
    ...
    transpose(self.state_matrix)
    return self.hex_matrix_to_text(self.state_matrix)   # Converting Hexadecimal matrix to text
